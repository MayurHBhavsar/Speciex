...
contract Swap {
    ...
    uint256 public totalAllotedAmount;
    uint256 hundred = 100;
    ...
    function buyTokens(uint256 _busdAmount) public {
        ...
        while (busdAmount > 0) {
            if (busdAmount >= 1000) {
                // transfer tokens for 1000 BUSD to user
                token.transfer(msg.sender, 1000 * tokensPerBUSD);
                busdAmount = busdAmount.sub(1000);
                // decreament tokensPerBUSD
                tokensPerBUSD = tokensPerBUSD.sub(tokensPerBUSD.mul(1).div(1000));
            } else {
                token.transfer(msg.sender, busdAmount * tokensPerBUSD);
                busdAmount = 0;
            }
        }
    ...
    function sellTokens(uint256 _tokenAmount) public {
        ...
        uint256 tokenAmount = _tokenAmount.mul(10**18);
        require(token.balanceOf(msg.sender) >= tokenAmount);

        while (tokenAmount > 0) {
            if (tokenAmount.div(tokensPerBUSD) >= 1000) {
                // transfer tokens worth 1000 BUSD
                token.transferFrom(msg.sender, address(this), 1000 * tokensPerBUSD);
                tokenAmount = tokenAmount.sub(1000 * tokensPerBUSD);
                // increament tokensPerBUSD
                tokensPerBUSD = tokensPerBUSD.add(tokensPerBUSD.mul(1).div(1000));
                // Transfer 1000 BUSD to user
                BUSD.transfer(msg.sender, 1000 * 10**18);
            } else {
                token.transferFrom(msg.sender, address(this), tokenAmount);
                // Transfer BUSD woth tokenAmount
                BUSD.transfer(msg.sender, tokenAmount.mul(10**18).div(tokensPerBUSD));
                tokenAmount = 0;
            }
        }
    }

    function getCurrentTokenRate() view public returns(uint256) {
        uint256 one = 10 ** 27;
        return one.div(tokensPerBUSD);
    }   
